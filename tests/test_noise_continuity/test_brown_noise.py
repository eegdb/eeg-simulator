...
布朗噪声 (Brown Noise / 1/f² 噪声) 是随机游走过程，具有 1/f² 频谱特性。
作为随机过程，不需要跨批次时域连续性。
...
def test_brown_noise_continuity(
    sampling_rate=1000,
    chunk_duration=0.5,
    n_chunks=10,
    save_path=None
    ...
    noise_config = {'type': 'brown', 'amplitude': 10.0}
    ...
    print("\n[1/2] 一次性生成 (参考基准)...")
    # 一次性生成完整的布朗噪声
    independent_signal = engine._generate_brown_noise(total_samples, 10.0)
    ...
    for i in range(n_chunks):
        noise = engine._generate_continuous_brown_noise(n_samples_per_chunk, 10.0, state)
    ...
    fig.suptitle('Brown Noise Continuity Test', fontsize=14, fontweight='bold')
    ...
    print(f"  独立生成 - 频谱斜率: {slope_ind:.2f} (理想: -2.0)")
    print(f"  连续生成 - 频谱斜率: {slope_con:.2f} (理想: -2.0)")
...
def main():
    ...
    results = test_brown_noise_continuity(
        sampling_rate=1000,
        chunk_duration=0.5,
        n_chunks=10,
